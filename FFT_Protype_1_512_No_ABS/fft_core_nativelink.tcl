...
catch {set testbench_files [glob fft_core*.hex]} error_msg
puts $error_msg
catch {set input_files [glob fft_core*input.txt]} error_msg
puts $error_msg
     
# The top-level in HDL type "Verilog"
set ipfs_ext vo
set hdl_ext v
if {[file exists fft_core.$ipfs_ext]} {
        puts "Info: Found fft_core.$ipfs_ext."
} else {
	puts "Warning: Could not find fft_core.$ipfs_ext!"
}
...
# IPFS file
set_global_assignment -name EDA_IPFS_FILE fft_core.$ipfs_ext -section_id eda_simulation -library work
...
set_global_assignment -name EDA_TEST_BENCH_FILE fft_core_tb.$hdl_ext -section_id tb -library work
